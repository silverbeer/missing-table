...
class AuthFactory(BaseFactory):
    ...
    @classmethod
    def auth_headers(cls, token: Optional[str] = None) -> Dict[str, str]:
...
if __name__ == "__main__":
    ...
    print(f"Users: {admin['role']}, {manager['role']}, {user['role']}")
    
    # Create a league table
    table = StandingsFactory.league_table(num_teams=8)
    print(f"League table with {len(table)} teams")
    
    # Load all seed data
    seed_data = load_all_seed_data()
    print(f"Loaded {len(seed_data['teams'])} teams from seed data")
