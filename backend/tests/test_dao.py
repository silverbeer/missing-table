import pytest
from backend.dao import sports_dao
...
def test_get_all_teams():
    db_conn_holder_obj = sports_dao.DbConnectionHolder(db_file='backend/mlsnext_u13_fall.db')
    dao = sports_dao.SportsDAO(db_conn_holder_obj)
...
def test_get_games_by_team(sports_dao):
    games = sports_dao.get_games_by_team('Test Team')
    assert isinstance(games, list)  # Check that the result is a list
    for game in games:
...
def test_add_game(sports_dao):
    sports_dao.add_game('2023-10-01', 'Test Team', 'Another Team', 2, 1)
    games = sports_dao.get_all_games()
