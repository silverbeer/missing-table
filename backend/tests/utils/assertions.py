...
def assert_valid_response(response, status_code: int = 200, content_type: str = "application/json"):
    """
...
def assert_error_response(response, status_code: int, error_message: str | None = None):
    """
    ...
    """
    assert response.status_code == status_code, f"Expected error status {status_code}, got {response.status_code}"
...
def assert_has_keys(data: dict[str, Any], required_keys: list[str]):
    ...
    assert not missing_keys, f"Data missing required keys: {missing_keys}. Available keys: {list(data.keys())}"


def assert_is_list(data: Any, min_length: int | None = None, max_length: int | None = None):
    """
    ...
    if min_length is not None:
        assert len(data) >= min_length, f"Expected list length >= {min_length}, got {len(data)}"

    if max_length is not None:
        assert len(data) <= max_length, f"Expected list length <= {max_length}, got {len(data)}"
...
def assert_is_valid_id(value: Any):
    ...
    if isinstance(value, int):
        ...
        raise AssertionError(f"Expected int or string ID, got {type(value)}: {value}")
...
def assert_valid_match(match: dict[str, Any], strict: bool = False):
    ...
    assert match["home_team_id"] != match["away_team_id"], "Home and away teams must be different"

    # Validate date format (basic check)
    assert len(match["match_date"]) >= 10, f"Invalid match_date format: {match['match_date']}"
...
def assert_valid_user(user: dict[str, Any], strict: bool = False):
    ...
    if "role" in user:
        ...
        assert user["role"] in valid_roles, f"Invalid role: {user['role']}. Must be one of {valid_roles}"
...
def assert_valid_standings(standings: list[dict[str, Any]]):
    ...
    for team_stats in standings:
        ...
        # Games should add up
        assert played == wins + draws + losses, f"Games don't add up: {played} != {wins} + {draws} + {losses}"

        # Points calculation (3 for win, 1 for draw)
        expected_points = wins * 3 + draws
        assert points == expected_points, f"Points calculation wrong: {points} != {expected_points}"
...
def assert_requires_auth(response):
    ...
    assert response.status_code in [401, 403], f"Expected 401 or 403 (auth required), got {response.status_code}"
...
def assert_date_format(date_str: str, format: str = "%Y-%m-%d"):
    ...
    try:
        ...
        raise AssertionError(f"Invalid date format '{date_str}'. Expected format: {format}") from None
...
def assert_all_match(items: list[Any], predicate):
    ...
    assert not failures, f"{len(failures)} items failed predicate. First failure: {failures[0] if failures else None}"
