...
class VisualRegression:
    ...
    def __init__(self, page: Page, baseline_dir: Path, diff_dir: Path, threshold: float = 0.01) -> None:
        """
    ...
    def compare(
        ...
        try:
            ...
            if baseline.size != current.size:
                logger.error(f"Size mismatch: baseline {baseline.size} vs current {current.size}")
                return False
            ...
            if diff_ratio > self.threshold:
                ...
                logger.error(f"Visual difference detected: {diff_ratio:.2%} (threshold: {self.threshold:.2%})")
