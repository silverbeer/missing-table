...
@contextmanager
def authenticated_client(test_client: TestClient, user_role: str = "team-fan", team_id: int | None = None):
    ...
    def test_standings_calculation_consistency(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            ...
            for team_standing in standings[:3]:  # Check first 3 teams
                # API may return 'team' or 'team_name', and 'played' or 'games_played'
                assert 'team' in team_standing or 'team_name' in team_standing, "Missing team/team_name in standings"
                assert 'played' in team_standing or 'games_played' in team_standing, "Missing games_played/played in standings"
                required_fields = ['wins', 'draws', 'losses', 'points']
    ...
    def test_team_game_relationship_consistency(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            ...
            for team in teams[:3]:
                ...
                team_games_response = test_client.get(f"/api/matches/team/{team_id}")
...
class TestFilteringAndPagination:
    ...
    def test_games_filtering_combinations(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            ...
            game_types_response = test_client.get("/api/match-types")  # Updated endpoint

            # Skip if endpoints not available (503 Service Unavailable)
            if seasons_response.status_code != 200:
                pytest.skip(f"Seasons endpoint returned {seasons_response.status_code}")
            if age_groups_response.status_code != 200:
                pytest.skip(f"Age groups endpoint returned {age_groups_response.status_code}")
            if game_types_response.status_code != 200:
                pytest.skip(f"Match types endpoint returned {game_types_response.status_code}")
            ...
            if seasons and age_groups and game_types:
                ...
                # Test single filters
                if season_id:
                    response = test_client.get(f"/api/matches?season_id={season_id}")
                    assert response.status_code == 200

                if age_group_id:
                    response = test_client.get(f"/api/matches?age_group_id={age_group_id}")
                    assert response.status_code == 200

                if game_type_id:
                    response = test_client.get(f"/api/matches?game_type_id={game_type_id}")
                    assert response.status_code == 200

                # Test combination filters
                if all([season_id, age_group_id, game_type_id]):
                    response = test_client.get(
                        f"/api/matches?season_id={season_id}&age_group_id={age_group_id}&game_type_id={game_type_id}"
                    )
                    assert response.status_code == 200
    ...
    def test_table_filtering_combinations(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            ...
            seasons_response = test_client.get("/api/seasons")
            age_groups_response = test_client.get("/api/age-groups")
            game_types_response = test_client.get("/api/match-types")  # Updated endpoint name

            # Skip if endpoints not available
            if seasons_response.status_code != 200 or age_groups_response.status_code != 200 or game_types_response.status_code != 200:
                pytest.skip("Reference data endpoints not available")
                ...
                if all([season_id, age_group_id, game_type_id]):
                    ...
                    if filtered_standings and base_standings:
                        for standing in filtered_standings:
                            assert 'team' in standing or 'team_name' in standing
                            assert 'points' in standing
                            assert 'played' in standing or 'games_played' in standing
    ...
    def test_pagination_limits(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            ...
            response = test_client.get("/api/matches?limit=999999")
            assert response.status_code == 200
            games = response.json()
...
class TestDataIntegrityAndValidation:
    ...
    def test_date_handling_consistency(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            games_response = test_client.get("/api/matches?limit=5")
            assert games_response.status_code == 200
            games = games_response.json()

            for game in games:
                game_date = game.get('game_date') or game.get('match_date')
                if game_date:
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_nonexistent_resource_handling(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            response = test_client.get("/api/matches/team/999999")
            assert response.status_code == 200
            games = response.json()
    ...
    def test_malformed_query_parameters(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            malformed_params = [
                "/api/matches?season_id=not_a_number",
                "/api/matches?age_group_id=abc",
                "/api/matches?game_type_id=1.5",
                "/api/matches?limit=not_a_number",
...
class TestBusinessLogicValidation:
    ...
    def test_game_score_logic(self, test_client: TestClient):
        ...
        with authenticated_client(test_client):
            games_response = test_client.get("/api/matches?limit=10")
            assert games_response.status_code == 200
            games = games_response.json()
