...
7. [AI-Assisted Test Generation (QE Plugin)](#ai-assisted-test-generation-qe-plugin)
...
## 🤖 AI-Assisted Test Generation (QE Plugin)

Test generation and coverage auditing are handled by the [qe plugin](https://github.com/silverbeer/qe-plugin) for Claude Code (`/qe` for coverage audits, `/generate-tests` for gap-closing, `@qe-engineer` agent). Configuration lives in `.claude/qe.yml` at the repo root; the endpoint test manifest is tracked in `.qe/test_manifest.json`.

AI-generated tests are marked with `@pytest.mark.ai_generated`:
...
The earlier CrewAI-based agent system was retired — see [docs/04-testing/crewai-experiment-retrospective.md](../../docs/04-testing/crewai-experiment-retrospective.md) for what it was and what we learned.
...
- **QE Plugin**: https://github.com/silverbeer/qe-plugin
