...
def migrate_data():
    ...
    for team in sqlite_teams:
        ...
        try:
            ...
            if 'duplicate' in str(e):
                ...
                existing = supabase.table('teams').select('id').eq('name', team['name']).single().execute()
    ...
    for game in sqlite_games:
        ...
        try:
            ...
            print(f"❌ Error migrating game {game['game_date']}: {game['home_team']} vs {game['away_team']} - {e}")
            games_skipped += 1
