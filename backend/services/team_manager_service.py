...
class TeamManagerService:
    ...
    def assign_team_manager(
        ...
        try:
            ...
            response = self.supabase.table("team_manager_assignments").insert(assignment_data).execute()
    ...
    def can_manage_team(self, user_id: str, team_id: int, age_group_id: int) -> bool:
        ...
        try:
            ...
            user_response = self.supabase.table("user_profiles").select("role").eq("id", user_id).single().execute()
    ...
    def remove_team_manager(self, user_id: str, team_id: int, age_group_id: int) -> bool:
        """
    ...
    def get_team_managers(self, team_id: int, age_group_id: int | None = None) -> list[dict]:
        """
        ...
        try:
            select_fields = (
                "*, user:user_profiles!team_manager_assignments_user_id_fkey"
                "(id, email, first_name, last_name), age_groups(name)"
            )
            query = self.supabase.table("team_manager_assignments").select(select_fields).eq("team_id", team_id)
