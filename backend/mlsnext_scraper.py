...
class MLSNextScraper:
    ...
    async def scrape_games(self, match_date: str = None, club: str = None, division: str = "Northeast"):
        """
        ...
        async with async_playwright() as p:
            ...
            try:
                print(f"🌐 Navigating to MLS Next U13 schedule page...")
    ...
    async def _set_division(self, page, division: str):
        ...
        try:
            ...
            division_selectors = [
                f'select[name*="division"], select[id*="division"]',
                f'div[class*="division"] select',
    ...
    async def _set_club(self, page, club: str):
        ...
        try:
            ...
            club_selectors = [
                f'select[name*="club"], select[id*="club"]',
                f'div[class*="club"] select',
                f'select option:has-text("{club}")',
                f'button:has-text("{club}")',
                f'input[placeholder*="club"]'
            ]
    ...
    async def _force_set_filters(self, page):
        ...
        try:
            ...
            for select in all_selects:
                ...
                first_option = await page.evaluate("select => select.options[0]?.text || ''", select)
                if "division" in first_option.lower() or "all" in first_option.lower():
                    options = await page.evaluate("select => Array.from(select.options).map(opt => opt.text)", select)
                    ...
                    for idx, opt in enumerate(options):
                        if "northeast" in opt.lower():
                            ...
                            await page.evaluate(f"select => {{ select.selectedIndex = {idx}; select.dispatchEvent(new Event('change')); }}", select)
                            break
            ...
            for selector in game_selectors:
                ...
                if count > 0:
                    ...
                    for i in range(count):
                        ...
                        if text_content and len(text_content.strip()) > 10:  # Has substantial content
                            valid_elements.append(element)
                    
                    if valid_elements:
                        game_elements = valid_elements
                        selector_used = selector
                        print(f"✅ Found {len(valid_elements)} potential game elements using selector: {selector}")
                        break
            ...
            for i, game_element in enumerate(game_elements):
                try:
                    ...
                    if game_data:
                        ...
                        print(f"   📝 Game {i+1}: {game_data.get('home_team', 'Unknown')} vs {game_data.get('away_team', 'Unknown')}")
                except Exception as e:
    ...
    async def _extract_single_game(self, game_element):
        ...
        try:
            ...
            team_elements = game_element.locator('div, span, td').filter(has_text=lambda text: len(text.strip()) > 3)
                ...
                for match in matches:
                    ...
                    if len(groups) >= 4:
                        game_data = {
                            ...
                            'away_team': groups[3].strip() if len(groups) > 3 else groups[1].strip(),
                        ...
                        if (len(game_data['home_team']) > 2 and len(game_data['away_team']) > 2 and
                            not any(word in game_data['home_team'].lower() for word in ['schedule', 'scores', 'filter', 'search'])):
                            games.append(game_data)
    ...
    async def scrape_date_range(self, start_date: str, end_date: str, club: str = None, division: str = "Northeast"):
    ...
    def save_to_csv(self, filename: str = None):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['game_date', 'home_team', 'away_team', 'home_score', 'away_score', 'status']
