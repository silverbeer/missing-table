...
print(
    f"U13: {u13_age_group_id}, Northeast: {northeast_division_id}, 2024-2025: {season_id}, League: {league_game_type_id}"
)

# Get all teams for U13 Northeast
teams_response = (
    supabase.table("team_mappings")
    .select("team_id, teams(id, name)")
    .eq("age_group_id", u13_age_group_id)
    .eq("division_id", northeast_division_id)
    .execute()
)
team_lookup = {}
...
print(
    f"  New games imported: {successful_batches * batch_size + (len(new_games) % batch_size if successful_batches > 0 else 0)}"
)

# Verify total game count
final_games_response = (
    supabase.table("games")
    .select("id")
    .eq("season_id", season_id)
    .eq("age_group_id", u13_age_group_id)
