...
async def create_invite_request(request: InviteRequestCreate):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="An error occurred while submitting your request") from None
...
async def list_invite_requests(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve invite requests") from None
...
async def get_invite_request(request_id: str, current_user=Depends(get_current_user_required)):
    """
    ...
    try:
        result = service_client.table("invite_requests").select("*").eq("id", request_id).execute()
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve invite request") from None
...
async def update_invite_request_status(
    ...
    try:
        # Get current request
        existing = service_client.table("invite_requests").select("*").eq("id", request_id).execute()
        ...
        raise HTTPException(status_code=500, detail="Failed to update invite request") from None


@router.delete("/{request_id}")
async def delete_invite_request(request_id: str, current_user=Depends(get_current_user_required)):
    """
    ...
    try:
        ...
        existing = service_client.table("invite_requests").select("id").eq("id", request_id).execute()
        ...
        return {"success": True, "message": "Invite request deleted successfully"}
