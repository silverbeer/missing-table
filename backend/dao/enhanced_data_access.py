...
class EnhancedSportsDAO:
    ...
    def add_team(self, name: str, city: str, age_group_ids: list[int]) -> bool:
        ...
        try:
            ...
            team_age_group_data = [
                {"team_id": team_id, "age_group_id": age_group_id} for age_group_id in age_group_ids
            ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        for game in games[:3]:  # Show first 3
            print(
                f"  - {game['game_date']}: {game['home_team']} vs {game['away_team']} ({game['home_score']}-{game['away_score']}) [{game['game_type']}]"
            )
