-- Migration: Fix RLS policies for matches table (COMPLETE - includes prerequisites)
-- Date: 2025-10-18
-- Issue: When games table was renamed to matches, RLS policies were not migrated
-- Impact: Team managers could not update matches for their teams (returning "no rows affected")
-- Fix: Create all helper functions and proper RLS policies for matches table
