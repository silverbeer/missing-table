...
> **Update (2025-11-28):** Backend security monitoring (security_monitor and auth_security_monitor) was removed. References in this guide remain for historical context only and should not be implemented in new code.
...
> The signup endpoint now uses structlog-based contextual logging (`audit_logger = logger.bind(...)`). Refer to `backend/app.py` for the authoritative implementation.

### Step 2.4: Update Login Endpoint

**File**: `backend/app.py` (line 381-450)

> Login instrumentation likewise binds contextual metadata to the structlog logger (`auth_logger = logger.bind(...)`) and records `auth_login_success` / `auth_login_error` events.
