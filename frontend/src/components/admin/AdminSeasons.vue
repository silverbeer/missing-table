    ...
    <div v-else class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
      <table class="min-w-full divide-y divide-gray-300">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="season in seasons" :key="season.id" :class="{ 'bg-blue-50': isCurrentSeason(season) }">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
              {{ season.name }}
              <span v-if="isCurrentSeason(season)" class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                Current
              </span>
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              ...
              <button
                ...
                :class="{ 'opacity-50 cursor-not-allowed': getGamesCount(season.id) > 0 }"
              >
                Delete
              </button>
            </td>
    ...
    <div v-if="showAddModal || showEditModal" class="modal-overlay" @click="closeModals">
      <div class="modal-content" @click.stop>
        <div class="p-6">
          ...
          <form @submit.prevent="showEditModal ? updateSeason() : createSeason()">
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-2">Season Name</label>
              <input
...
export default {
  ...
  setup() {
    ...
    const updateSeason = async () => {
      try {
        formLoading.value = true
        await authStore.apiRequest(`http://localhost:8000/api/seasons/${editingSeason.value.id}`, {
          method: 'PUT',
          body: JSON.stringify(formData.value)
        })
    ...
    const deleteSeason = async (season) => {
      ...
      try {
        await authStore.apiRequest(`http://localhost:8000/api/seasons/${season.id}`, {
          method: 'DELETE'
        })
