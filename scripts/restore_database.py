...
# Get DATABASE_URL for direct PostgreSQL connection
database_url = os.getenv("DATABASE_URL")

if not database_url:
    raise Exception("Missing required environment variable: DATABASE_URL")
...
def clear_table(table_name: str):
    """Clear all data from a table using direct SQL (bypasses RLS)."""
    try:
        print(f"Clearing {table_name}...")

        # Use TRUNCATE CASCADE for faster deletion (bypasses RLS)
...
def restore_table(table_name: str, data: list):
    """Restore data to a single table using direct SQL (bypasses RLS)."""
    if not data:
...
def reset_sequences():
    ...
    try:
        ...
        cursor.execute("SELECT reset_all_sequences();")
        result = cursor.fetchone()
        conn.commit()

        if result and result[0] is not None:
            sequences_reset = result[0]
...
def restore_from_backup(backup_file: Path, clear_existing: bool = True):
    ...
    restoration_order = [
        ...
        'age_groups',
        'leagues',  # Must come before divisions
        'clubs',    # Must come before teams (if present in backup)
        'divisions',  # Depends on leagues
        'match_types',  # Updated from game_types
        'seasons',

        # Teams (depend on divisions, age_groups, clubs, leagues)
        'teams',
        'team_mappings',
